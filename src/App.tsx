...
const isTestEnvironment =
  (globalThis as { __MARKDOWN_TO_PDF_TEST__?: boolean })
    .__MARKDOWN_TO_PDF_TEST__ === true;
...
const bodyAlignmentOptionLabels: Record<
  keyof typeof BODY_TEXT_ALIGNMENTS,
  string
> = {
...
const applyHeadingAlignmentToAll = (
  alignment: HeadingTextAlignment,
): StyleState["headingAlignments"] =>
...
const hasNonCompactListContent = (children: ReactNode): boolean =>
  Children.toArray(children).some((child) => {
    ...
    if (
      ...
      NON_COMPACT_LIST_TAGS.has(child.type)
    ) {
      return true;
    }

    if (
      typeof (child.props as { src?: unknown; ["data-missing-src"]?: unknown })
        .src === "string" ||
      typeof (child.props as { src?: unknown; ["data-missing-src"]?: unknown })[
        "data-missing-src"
      ] === "string"
    ...
    return hasNonCompactListContent(
...
const getTextContentLength = (children: ReactNode): number =>
  Children.toArray(children).reduce<number>((total, child) => {
    ...
    return (
      total +
      getTextContentLength((child.props as { children?: ReactNode }).children)
    );
  }, 0);

const buildListClassName = (children: ReactNode, className?: string) =>
  [className, isCompactList(children) ? "compact-list" : ""]
    .filter(Boolean)
    .join(" ") || undefined;
  ...
  return (
    ...
    listItems.every(
      (item) =>
        ...
        getTextContentLength(item.props.children) <=
          MAX_COMPACT_LIST_ITEM_TEXT_LENGTH,
    )
  );
};
...
const preloadDocumentImages = async (root: ParentNode) => {
  ...
  await Promise.all(
    Array.from(new Set(sources)).map((src) => preloadImageSource(src)),
  );
};
...
export function DocumentContent({
  ...
  const renderMarkdownImage = ({
    ...
    const nextClassName = ["document-image", className]
      .filter(Boolean)
      .join(" ");
    ...
    if (!resolvedSrc && src && isManagedImagePath(src)) {
      return (
        <span
          ...
          data-missing-src={normalizedManagedPath}
        >
      ...
      <article className="markdown-body">
        <ReactMarkdown
          ...
          remarkPlugins={[
            remarkGfm,
            remarkMath,
            [remarkDocumentMarkdownTransform, { markdown }],
          ]}
...
const PaletteColorField = memo(function PaletteColorField({
  ...
  const draftValue =
    draftState.controlledValueKey === controlledValueKey
...
function App() {
  ...
  const [pagePreset, setPagePreset] =
    useState<PagePresetKey>(DEFAULT_PAGE_PRESET);
  const [themePreset, setThemePreset] =
    useState<ThemeSelection>(DEFAULT_THEME_PRESET);
  const [horizontalMarginMm, setHorizontalMarginMm] = useState(
    DEFAULT_HORIZONTAL_MARGIN_MM,
  );
  const [verticalMarginMm, setVerticalMarginMm] = useState(
    DEFAULT_VERTICAL_MARGIN_MM,
  );
  const [pageChrome, setPageChrome] = useState(DEFAULT_PAGE_CHROME);
  const [isControlsExpanded, setIsControlsExpanded] = useState(false);
  const [debouncedMarkdown, setDebouncedMarkdown] = useState(markdown);
  const [stylesetNotice, setStylesetNotice] = useState<NoticeState | null>(
    null,
  );
  ...
  const [imagePathDrafts, setImagePathDrafts] = useState<
  ...
  const totalImageBytes = imageAssets.reduce(
  ...
  useEffect(() => {
    ...
    setImagePathDrafts((current) =>
      Object.fromEntries(
        imageAssets.map((asset) => [asset.id, current[asset.id] ?? asset.path]),
      ),
    );
  }, [imageAssets]);
  ...
  useEffect(() => {
    ...
    if (!hasPersistedMarkdownRef.current) {
      ...
      if (
        initialStoredMarkdownRef.current === null &&
  ...
  useEffect(() => {
    ...
    const renderPagedPreview = async () => {
      try {
        ...
        setPageCount(
          flow.total ?? container.querySelectorAll(".pagedjs_page").length,
        );
      } catch (error) {
        if (cancelled) {
          return;
    ...
    setStyleState((current) => {
      ...
      if (mode === "set") {
        return {
          ...
          headingAlignments: applyHeadingAlignmentToAll(
            current.headingAlignments.h1,
          ),
        };
      }
  ...
  const handleExportStyleset = () => {
    ...
    const blob = new Blob([serializeStylesetState(styleset)], {
  ...
  const insertImageReference = (asset: ImageAsset) => {
    ...
    const needsLeadingBreak =
      selectionStart > 0 &&
      !textarea.value
        .slice(Math.max(0, selectionStart - 2), selectionStart)
        .includes("\n");
    const needsTrailingBreak =
      selectionEnd < textarea.value.length &&
      !textarea.value
        .slice(selectionEnd, Math.min(textarea.value.length, selectionEnd + 2))
        .includes("\n");
      ...
      for (const file of files) {
        nextAssets.push(
          await createImageAssetFromFile(
            file,
            nextAssets.map((asset) => asset.path),
          ),
        );
      }
  ...
  const commitImagePathRename = (assetId: string, draftValue?: string) => {
    ...
    const nextPath = buildUniqueImageAssetPath(
      ...
      currentAssets
        ...
        .map((currentAsset) => currentAsset.path),
    );
    ...
    if (
      ...
      replaceEditorMarkdown(
        replaceImageAssetPathReferences(markdown, asset.path, nextPath),
      );
    }
  };
  ...
  const displayMathAlignmentLayout = getDisplayMathAlignmentLayout(
    styleState.displayMathAlignment,
  );
  const previewDocumentStyle = {
    ...
    "--page-heading-family":
      HEADING_FONT_PRESETS[styleState.headingFamily].family,
    ...
    "--page-table-cell-vertical-padding": `${styleState.tableCellVerticalPaddingRem}rem`,
    "--page-table-cell-horizontal-padding": `${styleState.tableCellHorizontalPaddingRem}rem`,
    ...
    "--page-blockquote-padding-right":
      bodyAlignmentLayout.blockquotePaddingRight,
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm text-[var(--chrome-muted)]">
                    All assets are stored locally in your browser. Refer with
                    paths like <code>assets/diagram.png</code>.
                  </p>
                </div>
                <button
                ...
                >
                  <div>
                    ...
                    <p className="m-0 mt-1 text-sm text-[var(--chrome-muted)]">
                      Upload PNG, JPG, GIF, SVG, or WebP and insert it into the
                      Markdown editor by path.
                    </p>
                  </div>
                      ...
                      <div className="min-w-0">
                        ...
                        <label className="mt-2 grid gap-1.5">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              handleImagePathDraftChange(
                                asset.id,
                                event.target.value,
                              )
                            }
                            onBlur={(event) =>
                              commitImagePathRename(
                                asset.id,
            ...
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              <div className="min-w-0">
                ...
                {/*<p className="mt-1.5 max-w-[70ch] text-sm text-[var(--chrome-muted)] sm:text-[0.95rem]">
                  Write markdown, preview, and export a polished PDF directly
                  from the browser.
                </p>*/}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 xl:justify-end">
                ...
                <button
                  ...
                  onClick={handlePrint}
                  disabled={
                    isPaginating || !!paginationError || pageCount === 0
                  }
                >
                  Print / Save PDF
                </button>
                  ...
                  <div className={controlPanelClass}>
                    <section className="flex flex-col gap-3 rounded-[1rem] border border-white/8 bg-black/15 p-3">
                      ...
                      <label className="grid gap-1.5">
                        ...
                        <SelectField
                          ...
                          value={pagePreset}
                          onChange={(event) =>
                            setPagePreset(event.target.value as PagePresetKey)
                          }
                        >
                      ...
                      <div className="grid gap-3">
                        <label className="grid gap-1.5">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              setHorizontalMarginMm(Number(event.target.value))
                            }
                          />
                        ...
                        <label className="grid gap-1.5">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              setVerticalMarginMm(Number(event.target.value))
                            }
                          />
                          <span className="text-sm text-[var(--chrome-muted)]">
                            {verticalMarginMm} mm
                        ...
                        <label className="grid gap-1.5">
                          ...
                          <input
                            aria-label="Table horizontal padding"
                            className="w-full accent-[var(--chrome-accent)]"
                            type="range"
                            min={MIN_TABLE_CELL_PADDING_REM}
                            max={MAX_TABLE_CELL_PADDING_REM}
                            step="0.05"
                            value={styleState.tableCellHorizontalPaddingRem}
                            onChange={(event) =>
                              updateStyle("tableCellHorizontalPaddingRem")(
                                Number(event.target.value),
                              )
                            }
                          />
                          <span className="text-sm text-[var(--chrome-muted)]">
                            {styleState.tableCellHorizontalPaddingRem.toFixed(
                              2,
                            )}{" "}
                            rem
                          </span>
                        ...
                        <label className="grid gap-1.5">
                          ...
                          <input
                            aria-label="Table vertical padding"
                            className="w-full accent-[var(--chrome-accent)]"
                            type="range"
                            min={MIN_TABLE_CELL_PADDING_REM}
                            max={MAX_TABLE_CELL_PADDING_REM}
                            step="0.05"
                            value={styleState.tableCellVerticalPaddingRem}
                            onChange={(event) =>
                              updateStyle("tableCellVerticalPaddingRem")(
                                Number(event.target.value),
                              )
                            }
                          />
                          <span className="text-sm text-[var(--chrome-muted)]">
                            {styleState.tableCellVerticalPaddingRem.toFixed(2)}{" "}
                            rem
                          </span>
                        </label>
                      </div>
                    ...
                    <section className="flex flex-col gap-3 rounded-[1rem] border border-white/8 bg-black/15 p-3">
                        ...
                        <SelectField
                          ...
                          onChange={(event) =>
                            updateStyle("fontFamily")(
                              event.target.value as StyleState["fontFamily"],
                            )
                          }
                        >
                          {Object.entries(BODY_FONT_PRESETS).map(
                            ([key, preset]) => (
                              <option key={key} value={key}>
                                {preset.label}
                              </option>
                      ...
                      <label className="grid gap-1.5">
                        ...
                        >
                          {Object.entries(HEADING_FONT_PRESETS).map(
                            ([key, preset]) => (
                              <option key={key} value={key}>
                                {preset.label}
                              </option>
                          ...
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="grid grid-cols-2 gap-1 rounded-lg border border-white/10 bg-black/20 p-1">
                              {(
                                Object.entries(
                                  HEADING_ALIGNMENT_MODES,
                                ) as Array<[HeadingAlignmentMode, string]>
                              ).map(([key, label]) => {
                                const isSelected =
                                  styleState.headingAlignmentMode === key;
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={() =>
                                      updateHeadingAlignmentMode(key)
                                    }
                                  >
                                    {label}
                                  </button>
                            ...
                            >
                                ...
                                HEADING_ALIGNMENT_KEYS.map((level) => (
                                  <div key={level} className="grid gap-1.5">
                                    ...
                                    <AlignmentSelector
                                      ...
                                      value={
                                        styleState.headingAlignments[level]
                                      }
                                      onChange={(nextAlignment) =>
                                        updateCustomHeadingAlignment(
                                          level,
                                          nextAlignment,
                                        )
                                      }
                              ...
                              <div className="grid gap-1.5">
                                ...
                                <AlignmentSelector
                                  ...
                                  onChange={(nextAlignment) =>
                                    updateStyle("displayMathAlignment")(
                                      nextAlignment,
                                    )
                                  }
                                  options={Object.entries(
                    ...
                    <section className="flex flex-col gap-3 rounded-[1rem] border border-white/8 bg-black/15 p-3">
                      ...
                      <div className="flex flex-col gap-2 rounded-xl border border-white/8 bg-black/10 p-3">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          <label className="flex items-center gap-2 text-sm font-semibold">
                            <input
                              ...
                              onChange={(event) =>
                                updatePageChrome("headerEnabled")(
                          ...
                          <SelectField
                            ...
                            onChange={(event) =>
                              updatePageChrome("headerPosition")(
                                event.target
                                  .value as PageChromeState["headerPosition"],
                              )
                            }
                          >
                            {Object.entries(HEADER_POSITIONS).map(
                              ([key, label]) => (
                                <option key={key} value={key}>
                                  {label}
                                </option>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updatePageChrome("headerText")(event.target.value)
                          }
                        />
                      </div>
                      <div className="flex flex-col gap-2 rounded-xl border border-white/8 bg-black/10 p-3">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          <label className="flex items-center gap-2 text-sm font-semibold">
                            <input
                              ...
                              onChange={(event) =>
                                updatePageChrome("footerEnabled")(
                                  event.target.checked,
                                )
                              }
                            />
                          ...
                          <SelectField
                            ...
                            onChange={(event) =>
                              updatePageChrome("footerPosition")(
                                event.target
                                  .value as PageChromeState["footerPosition"],
                              )
                            }
                          >
                            {Object.entries(FOOTER_POSITIONS).map(
                              ([key, label]) => (
                                <option key={key} value={key}>
                                  {label}
                                </option>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updatePageChrome("footerText")(event.target.value)
                          }
                        />
                      </div>
                      <div className="flex flex-col gap-2 rounded-xl border border-white/8 bg-black/10 p-3">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          <label className="flex items-center gap-2 whitespace-nowrap text-sm font-semibold">
                            <input
                              ...
                              onChange={(event) =>
                                updatePageChrome("pageNumbersEnabled")(
                                  event.target.checked,
                                )
                              }
                            />
                          ...
                          <SelectField
                            ...
                            onChange={(event) =>
                              updatePageChrome("pageNumberPosition")(
                                event.target
                                  .value as PageChromeState["pageNumberPosition"],
                              )
                            }
                          >
                            {Object.entries(PAGE_NUMBER_POSITIONS).map(
                              ([key, label]) => (
                                <option key={key} value={key}>
                                  {label}
                                </option>
                      ...
                      <div className="grid gap-3 rounded-xl border border-white/8 bg-black/10 p-3 sm:grid-cols-2">
                        <label className="grid gap-1.5">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              updatePageChrome("headerFontSizePt")(
                                Number(event.target.value),
                              )
                            }
                          />
                        ...
                        <label className="grid gap-1.5">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              updatePageChrome("footerFontSizePt")(
                                Number(event.target.value),
                              )
                            }
                          />
                        ...
                        {Object.entries(THEME_PRESETS).map(([key, preset]) => {
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                handleThemePresetSelect(key as ThemePresetKey)
                              }
                            >
                              {preset.label}
                            </button>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {MARKDOWN_ACTIONS.map((action) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      applyToolbarAction(action.key as MarkdownActionKey)
                    }
                  >
                    {action.label}
                  </button>
