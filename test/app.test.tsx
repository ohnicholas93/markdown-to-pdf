...
describe("App", () => {
  ...
  test("renders the default markdown preview and default stats", async () => {
    ...
    try {
      ...
      expect(
        view.getByText(`${countWords(editor.value)} words`),
      ).toBeInTheDocument();
      expect(view.queryByText("Real print rendering")).toBeNull();
      expect(
        view.queryByText("Bun + Vite client-side print studio"),
      ).toBeNull();
  ...
  test("keeps document settings collapsed until requested", () => {
    ...
    const horizontalPadding = view.getByLabelText("Table horizontal padding");
    const verticalPadding = view.getByLabelText("Table vertical padding");

    expect(horizontalPadding).toBeInTheDocument();
    expect(verticalPadding).toBeInTheDocument();
    expect(
      horizontalPadding.compareDocumentPosition(verticalPadding) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });

  test("allows smaller heading and body type sizes", () => {
    const view = render(<App />);

    fireEvent.click(view.getByText("Document Settings"));

    expect(view.getByLabelText("Heading base size")).toHaveAttribute(
      "min",
      "12",
    );
    expect(view.getByLabelText("Body font size")).toHaveAttribute("min", "12");
    expect(view.getByLabelText("Heading base size")).toHaveValue("14");
    expect(view.getByLabelText("Body font size")).toHaveValue("14");
    expect(view.getByLabelText("Table horizontal padding")).toHaveValue("0.6");
    expect(view.getByLabelText("Table vertical padding")).toHaveValue("0.35");
  });
  ...
  test("supports keyboard resizing and clamps the split ratio", () => {
    ...
    expect(
      parseFloat(main.style.getPropertyValue("--editor-width")),
    ).toBeCloseTo(42);
  ...
  test("uses stable list alignment and explicit page breaks in the print stylesheet", () => {
    ...
    expect(css).toContain(
      "text-align: var(--page-list-text-align, var(--page-body-align));",
    );
    expect(css).toContain(
      "text-align-last: var(--page-list-text-align, var(--page-body-align));",
    );
    ...
    expect(css).not.toContain(
      ".markdown-body tbody tr:last-child > :first-child",
    );
    expect(css).not.toContain(".markdown-body tbody tr:last-child > *");
    expect(css).toContain(
      ".markdown-body mjx-container[jax='SVG'] path[data-c],",
    );
  ...
  test("does not reset markdown when reset confirmation is cancelled", () => {
    ...
    try {
      ...
      expect(editor.value).toBe("# Saved draft\n\nPersisted copy");
      expect(localStorage.getItem(MARKDOWN_STORAGE_KEY)).toBe(
        "# Saved draft\n\nPersisted copy",
      );
    } finally {
      window.confirm = originalConfirm;
  ...
  test("toggles the image library sidebar from the editor pane", () => {
    ...
    expect(view.getByRole("button", { name: "Open image library" })).toHaveAttribute(
      "aria-expanded",
      "false",
    );
    expect(sidebar).toHaveAttribute("aria-hidden", "true");
  ...
  test("renders markdown lists, raw html, and signature lines in the live preview", () => {
    ...
    expect(
      view.container.querySelector(".signature-line")?.getAttribute("style"),
    ).toBe("width: 12ch;");
  });
  ...
  test("renders managed images, captions, and missing-image placeholders in the live preview", () => {
    ...
    expect((view.getByAltText("Cover") as HTMLImageElement).src).toContain(
      imageAsset.dataUrl,
    );
  ...
  test("marks short simple lists as compact and leaves loose lists splittable", () => {
    ...
    expect(compact.container.querySelector("ul")?.className).toContain(
      "compact-list",
    );
    expect(loose.container.querySelector("ul")?.className ?? "").not.toContain(
      "compact-list",
    );
    expect(
      longTight.container.querySelector("ul")?.className ?? "",
    ).not.toContain("compact-list");
    expect(media.container.querySelector("ul")?.className ?? "").not.toContain(
      "compact-list",
    );
    expect(
      hardBreaks.container.querySelector("ul")?.className ?? "",
    ).not.toContain("compact-list");
  });
...
\end{align}`}
  ...
  test("theme selection is not overwritten by a throttled palette sync", () => {
    ...
    expect((view.getByLabelText("Paper") as HTMLInputElement).value).toBe(
      "#e8ecf3",
    );
  ...
  test("exports the current styleset as JSON", async () => {
    ...
    await waitFor(() => {
      expect(
        (view.getByLabelText("Header size") as HTMLInputElement).value,
      ).toBe("12");
    });
    ...
    expect(exported.style.tableCellVerticalPaddingRem).toBe(1.1);
    expect(exported.style.tableCellHorizontalPaddingRem).toBe(1.25);
  ...
  test("imports a styleset JSON file", async () => {
      ...
      [
        JSON.stringify({
          ...
          style: {
            ...
            tableCellVerticalPaddingRem: 1.2,
            tableCellHorizontalPaddingRem: 1.4,
            letterSpacing: 0.02,
            background: "#f4efe8",
            text: "#221f1c",
            accent: "#bf5b2c",
    ...
    await waitFor(() => {
      expect((view.getByLabelText("Page") as HTMLSelectElement).value).toBe(
        "legal",
      );
    });

    expect(
      (view.getByLabelText("Horizontal margin") as HTMLInputElement).value,
    ).toBe("22");
    expect(
      (view.getByLabelText("Vertical margin") as HTMLInputElement).value,
    ).toBe("24");
    expect((view.getByLabelText("PDF title") as HTMLInputElement).value).toBe(
      "Imported Portfolio",
    );
    expect((view.getByLabelText("Body font") as HTMLSelectElement).value).toBe(
      "space",
    );
    expect(
      (view.getByLabelText("Heading font") as HTMLSelectElement).value,
    ).toBe("playfair");
    ...
    expect(
      view.getByRole("radio", { name: "H1 center aligned" }),
    ).toHaveAttribute("aria-checked", "true");
    expect(
      view.getByRole("radio", { name: "Block LaTeX right aligned" }),
    ).toHaveAttribute("aria-checked", "true");
    expect(
      view.getByRole("radio", { name: "Justify body text" }),
    ).toHaveAttribute("aria-checked", "true");
    expect((view.getByLabelText("Paper") as HTMLInputElement).value).toBe(
      "#f4efe8",
    );
    expect((view.getByLabelText("Header text") as HTMLInputElement).value).toBe(
      "Imported Header",
    );
    expect(
      (view.getByLabelText("Table vertical padding") as HTMLInputElement).value,
    ).toBe("1.2");
    expect(
      (view.getByLabelText("Table horizontal padding") as HTMLInputElement)
        .value,
    ).toBe("1.4");
    expect((view.getByLabelText("Header size") as HTMLInputElement).value).toBe(
      "13",
    );
    expect((view.getByLabelText("Footer size") as HTMLInputElement).value).toBe(
      "10",
    );
    expect(
      view.getByText("Imported styleset from custom-styleset.json."),
    ).toBeInTheDocument();
  });
  ...
  test("imports legacy stylesets that only specify one margin value", async () => {
    ...
    expect(
      view.getByRole("radio", { name: "Grouped heading left aligned" }),
    ).toHaveAttribute("aria-checked", "true");
  ...
  test("switching from custom heading mode to set mode copies the h1 alignment to all headings", () => {
    ...
    expect(
      view.getByRole("radio", { name: "H3 center aligned" }),
    ).toHaveAttribute("aria-checked", "true");

    fireEvent.click(view.getByRole("button", { name: "Grouped" }));

    expect(
      view.getByRole("radio", { name: "Grouped heading right aligned" }),
    ).toHaveAttribute("aria-checked", "true");

    fireEvent.click(view.getByRole("button", { name: "Individual" }));

    expect(
      view.getByRole("radio", { name: "H1 right aligned" }),
    ).toHaveAttribute("aria-checked", "true");
    expect(
      view.getByRole("radio", { name: "H3 right aligned" }),
    ).toHaveAttribute("aria-checked", "true");
  });
  ...
  test("uploads, inserts, renames, and removes local images from the editor library", async () => {
    ...
    try {
      ...
      const file = new File(["image-binary"], "diagram.png", {
        type: "image/png",
      });
      ...
      await waitFor(() =>
        expect(
          view.getByDisplayValue("assets/diagram.png"),
        ).toBeInTheDocument(),
      );

      expect(localStorage.getItem(IMAGE_LIBRARY_STORAGE_KEY)).toContain(
        "assets/diagram.png",
      );
      ...
      const pathField = view.getByDisplayValue(
        "assets/diagram.png",
      ) as HTMLInputElement;
      ...
      await waitFor(() =>
        expect(
          view.getByDisplayValue("assets/cover-shot.png"),
        ).toBeInTheDocument(),
      );
  ...
  test("shows an error when a styleset JSON file is invalid", async () => {
    ...
    await waitFor(() => {
      expect(
        view.getByText("Could not import that styleset JSON file."),
      ).toBeInTheDocument();
    });
  });
});
